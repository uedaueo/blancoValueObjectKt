...
/**
 * バッチ処理クラス [BlancoValueObjectKtBatchProcess]。
 *
 * <P>バッチ処理の呼び出し例。</P>
 * <code>
 * java -classpath (クラスパス) blanco.valueobjectkt.task.BlancoValueObjectKtBatchProcess -help
 * </code>
 */
public class BlancoValueObjectKtBatchProcess {
    ...
    public static final void main(final String[] args) {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoValueObjectKtBatchProcess: 入力パラメータ[" + arg + "]は無視されました。");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoValueObjectKtBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
    ...
    public int process(final BlancoValueObjectKtProcessInput input) throws IOException, IllegalArgumentException {
        ...
        // この箇所でコンパイルエラーが発生する場合、BlancoValueObjectKtProcessインタフェースを実装して blanco.valueobjectkt.taskパッケージに BlancoValueObjectKtProcessImplクラスを作成することにより解決できる場合があります。
    ...
    public final int execute(final BlancoValueObjectKtProcessInput input) throws IllegalArgumentException {
        try {
            ...
            System.out.println("BlancoValueObjectKtBatchProcess: 入力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoValueObjectKtBatchProcess: 入出力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoValueObjectKtBatchProcess: ランタイム例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            ex.printStackTrace();
            // 異常終了。
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoValueObjectKtBatchProcess: ランタイムエラーが発生しました。バッチ処理を中断します。:" + er.toString());
            er.printStackTrace();
            // 異常終了。
            return END_ERROR;
        }
    ...
    public void validateInput(final BlancoValueObjectKtProcessInput input) throws IllegalArgumentException {
        ...
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoValueObjectKtBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
        }
    }
}
